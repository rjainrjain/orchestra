...
url <- "https://www.kcsymphony.org/about-us/musicians/"
data <- data.frame(section=NULL,name=NULL,link=NULL,title=NULL)


blocks <- read_html(url) %>% html_nodes(".instrument-section")
title <- blocks %>% html_nodes("span") %>% html_text()
title <- str_trim(str_remove_all(title,"\\n"))

count <- 1
for (i in 1:length(blocks)) {
  items <- blocks[i] %>% html_nodes(".musician-info")
  for (j in 1:length(items)){
    name <- items[j] %>% html_nodes(".musician-name") %>% html_text()
    if (length(name) == 0) {
      name <- NA
    }
    name <- str_trim(str_remove_all(name,"\\n"))
    link <- items[j] %>% html_nodes(".musician-name") %>% html_attr("href")
    if (length(link) == 0) {
      link <- NA
    }
    section <- blocks[i] %>% html_nodes("h5") %>% html_text()
